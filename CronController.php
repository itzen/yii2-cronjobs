...
RAW;
    ...
    public function actionRun($args = array())
    {
        ...
        foreach ($this->prepareActions() as $task) {
            if (array_intersect($tags, $task['docs']['tags'])) {
                ...
                $stderr = isset($task['docs']['stderr']) ? $this->formatFileName(
                    $task['docs']['stderr'],
                    $task
                ) : $stdout;
    ...
    public function actionView($args = array())
    {
        ...
        foreach ($this->prepareActions() as $task) {
            if (!$tags || array_intersect($tags, $task['docs']['tags'])) {
                ...
                array_push(
                    $times,
                    empty($task['docs']['tags']) ? '' : (' (' . implode(', ', $task['docs']['tags']) . ')')
                );
    ...
    protected function prepareActions()
    {
        $actions = array();
        try {
            $methods = $this->cronJobs;
        } catch (Exception $e) {
            throw new Exception('Empty param cronJobs in params. ', 8);
        }
